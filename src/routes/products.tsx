  ...
  return (
    <>
      ...
      <section className="py-20 bg-bone">
        <div className="container-wide grid gap-px bg-onyx/10 sm:grid-cols-2 lg:grid-cols-3 border border-onyx/10">
          {PRODUCTS.map((p, i) => (
            <Reveal key={p.slug} delay={i * 0.04}>
              <Link to="/products/$slug" params={{ slug: p.slug }} className="group block bg-bone p-8 h-full">
                <div className="aspect-[4/3] overflow-hidden">
                  <img src={p.image} alt={p.name} loading="lazy" className="h-full w-full object-cover transition-transform duration-[1200ms] group-hover:scale-110" />
                </div>
                <div className="pt-6 flex items-start justify-between">
                  <div>
                    ...
                    <h2 className="font-display text-2xl text-onyx">{p.name}</h2>
                    <p className="text-sm text-onyx/55 mt-2 max-w-xs">{p.tagline}</p>
                  </div>
                  <ArrowRight className="size-5 text-onyx/40 group-hover:text-gold group-hover:translate-x-1 transition-all" />
                </div>
              </Link>
            </Reveal>
          ))}
