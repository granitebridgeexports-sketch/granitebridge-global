...
const COUNTRIES: Record<string, { name: string; port: string; intro: string; ports: string }> = {
  germany: { name: "Germany", port: "Hamburg", intro: "Germany is the most demanding granite market in Europe — precise tolerances, full documentation, ISPM-15 packaging, and exacting finish standards. We've been shipping container loads to German importers, kitchen fabricators, and monument manufacturers for years.", ports: "Hamburg, Bremerhaven, Wilhelmshaven" },
  spain: { name: "Spain", port: "Valencia", intro: "Spain combines hospitality, residential developer demand, and a strong wholesale distribution network. We supply slabs, tiles, and cut-to-size to importers from Valencia to Bilbao.", ports: "Valencia, Barcelona, Algeciras, Bilbao" },
  italy: { name: "Italy", port: "Genoa", intro: "Italy is the global benchmark for stone — and that's exactly why precision-cut, premium-finished Indian granite from GraniteBridge competes here on quality, not just price.", ports: "Genoa, Livorno, La Spezia, Naples" },
  france: { name: "France", port: "Le Havre", intro: "We supply French importers and contractors across the Atlantic and Mediterranean coasts, with documentation prepared for French customs and TVA reporting.", ports: "Le Havre, Marseille, Fos-sur-Mer" },
  netherlands: { name: "Netherlands", port: "Rotterdam", intro: "Rotterdam is Europe's largest port and our key Benelux distribution gateway. Containers cleared and onward-shipped across northern Europe within days.", ports: "Rotterdam, Amsterdam" },
  belgium: { name: "Belgium", port: "Antwerp", intro: "Belgium remains one of Europe's strongest monument and memorial markets. We supply finished monument stone, headstones, and bases to Belgian importers and stonemasons.", ports: "Antwerp, Zeebrugge" },
  cyprus: { name: "Cyprus", port: "Limassol", intro: "Cyprus's resort, marina, and luxury villa construction relies on premium imported stone — we ship direct from Indian ports with minimal transit.", ports: "Limassol, Larnaca" },
  ...
  return (
    <>
      <section className="relative pt-40 pb-24 bg-onyx text-bone overflow-hidden">
        <img src={shippingImg} alt="" className="absolute inset-0 h-full w-full object-cover opacity-25" />
        <div className="absolute inset-0 bg-gradient-to-b from-onyx via-onyx/85 to-onyx" />
        <div className="container-wide relative">
          <div className="flex items-center gap-3 mb-6"><span className="hairline" /><span className="eyebrow">Indian Granite Exporter · {c.name}</span></div>
          <h1 className="font-display text-5xl lg:text-7xl max-w-4xl leading-[1]">
            Premium Indian granite, <span className="italic text-gold font-light">shipped to {c.name}.</span>
          </h1>
      ...
      <section className="bg-onyx text-bone py-24">
        <div className="container-wide grid gap-16 lg:grid-cols-12">
          <Reveal className="lg:col-span-5">
            ...
            <p className="mt-6 text-bone/65">Tell us your destination port and requirements. Our export desk responds within 24 hours.</p>
          </Reveal>
